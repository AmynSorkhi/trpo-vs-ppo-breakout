...
class TRPO:
    def __init__(self, env, gamma=0.995, lam=0.97, delta=0.2, cg_iters=10, cg_damping=0.1):
        ...
        self.best_mean_reward = -np.inf  # Track best mean reward

    def train(self, epochs=1000, steps_per_epoch=500, reward_threshold=2.0):# NOTE was 50 changed to 200 / 500, best result with 50 so far.
        ...
        log_filename = "training_log" + time.strftime("_%Y%m%d_%H%M%S_") + random_string() + ".csv"
        csvfile = open(log_filename, "w", newline="")
        csvwriter = csv.writer(csvfile)
        # Write header row including hyperparameters.
        csvwriter.writerow([
            "Epoch", "Mean Reward", "Std Reward", "Mean Rollout Length", "Total Steps",
            "Sample Efficiency", "KL", "Entropy", "Epoch Time",
        ...
        for epoch in range(epochs):
            ...
            for _ in range(steps_per_epoch):
                ...
                # print("Obs shape after reset:", state.shape)
                ...
                while not (done or truncated):
                    ...
                    # print("Obs shape after step:", next_state.shape)
                    total_steps += 1
            ...
            log_line = [
            epoch, mean_reward, std_reward, mean_length, total_steps,
            sample_efficiency, kl_metric, entropy_metric, epoch_duration
            ]
            ...
            # Write current epoch metrics to CSV
    ...
    def update_policy(self, states, actions, advantages):
        ...
        def line_search():
            for i, alpha in enumerate([0.5 ** i for i in range(20)]):
                ...
                tol = 1e-2  # or another small value NOTE try changing?
...
def parse_args():
    ...
    parser.add_argument("--steps_per_epoch", type=int, default=20, help="Number of steps per epoch.")
...
def load_and_run(checkpoint_path, num_episodes=5, render=True):
    """Load saved weights and run the environment."""
...
if __name__ == "__main__":
    # load_and_run('best_checkpoint.pth', num_episodes=5, render=True)
    # load_and_run('last_checkpoint.pth', num_episodes=5, render=True)
    ...
    trpo = TRPO(env, gamma=args.gamma, lam=args.lam, delta=args.delta, cg_iters=args.cg_iters, cg_damping=args.cg_damping)
    trpo.train(epochs=args.epochs, steps_per_epoch=args.steps_per_epoch, reward_threshold=args.reward_threshold)
